...
async function fixSchoolNames() {
  ...
  try {
    ...
    if (demands.length > 0) {
      console.log('表结构示例:', Object.keys(demands[0]))
    }
    ...
    for (const demand of demands) {
      const schoolId = demand.organization || '2'
    ...
    if (updatedError) {
      ...
      updatedDemands.forEach((demand, index) => {
        console.log(`  ${index + 1}. ID: ${demand.id}, 学校: ${demand.organization || '无'}, 学科: ${demand.subject || '无'}`)
      })
    }
